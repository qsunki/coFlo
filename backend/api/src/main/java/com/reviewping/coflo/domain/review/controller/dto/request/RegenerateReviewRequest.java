...
public record RegenerateReviewRequest(
        @NotNull Long projectId, @NotNull Long gitlabMrIid, @NotNull @Valid List<RetrievalContent> retrievals) {
    public record RetrievalContent(
            String content,
            String fileName,
            @EnumValue(enumClass = LanguageType.class, message = "지원하지 않는 언어입니다.", ignoreCase = true)
                    String language) {}
}
