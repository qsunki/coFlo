...
public class LinkService {
	...
	public List<GitlabProjectResponse> getGitlabProjects(Long userId, String keyword) {
		...
		List<GitlabProjectContent> searchGitlabProjects = gitLabApiService.searchGitlabProjects(
	...
	private GitlabAccount findGitlabAccount(Long userId) {
		User user = userRepository.findById(userId).orElseThrow(() -> new BusinessException(USER_NOT_EXIST));
		return gitlabAccountRepository.findFirstByUserIdOrderByIdAsc(user.getId())
